...
from retrieval_log_audit import __version__
from retrieval_log_audit.core import (
    audit_records,
    read_records,
    render_json,
    render_markdown,
...
from retrieval_log_audit.rules import PROJECT_NAME


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog='retrieval-log-audit',
        description=(
                        'Find retrieval logs with empty hits, weak scores, and missing source met'
                        'adata.'
                    ),
    )
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        help='retrieval log JSONL, CSV, or notes',
    )
    parser.add_argument(
        "--format",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"retrieval-log-audit: error: {exc}", file=sys.stderr)
        return 1
